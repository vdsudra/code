...
int main()
{
    ...
    while(running) 
    {
        ...
        {
            sleep(1);            
            //printf("waiting for reader to complate...\n");
        }
        printf("Enter some text(writer1):"); //if reader is free then ask to enter a data 
        fgets(buffer,BUFSIZ,stdin);//take data into buffer
        strncpy(shared_stuff->some_text,buffer,TEXT_SZ);//copy data to shared memory
        shared_stuff->written_by_you=1; // set read flag to 1 so reader can read
