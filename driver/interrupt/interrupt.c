...
#define IRQ 376
static int irq = IRQ, dev_id, count = 0;
module_param(irq, int, S_IRUGO);

static irqreturn_t my_handler(int irq, void *dev_id){
	count++;
	printk(KERN_INFO"Keyboard hit %d times", count);
	return IRQ_NONE;

}
int __init  my_init(void){
	if(request_irq(irq, my_handler, IRQF_SHARED, "my_interrupt", &dev_id))
		return -1;
	printk(KERN_INFO"Successful loading handler..\n");
	return 0;
}
...
void __exit my_exit(void){
	synchronize_irq(irq);
	free_irq(irq, &dev_id);
	printk(KERN_INFO"Successfuly unload handler...\n");
}
